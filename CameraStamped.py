...
RECORDING_PATH = "/home/pi/Pictures"  # Path where to shot footage
SERIAL_PORT = '/dev/ttyS0'  # '/dev/ttyS0' or '/dev/ttyAMA0'
   ...
   '''
    ...
    def run(self):
        ...
        while not self.shutdown_flag.is_set():
            ...
            hour = timenow.strftime("%H%M%S") + "." + timenow.strftime("%f")[:2]
            date = timenow.strftime("%d%m%y")
            time.sleep(0.1)
            print(date)
            first_line = "GPRMC," + hour + ",A,5227.42504,N,01317.81601,E,0.120,," + date + ",,,A*71\n\r"
            second_line = "GPVTG,,T,,M,0.120,N,0.223,K,A*23\n\r"
            third_line = "GPGGA," + hour + ",5227.42504,N,01317.81601,E,1,06,1.14,74.6,M,42.3,M,,*65\n\r"
            final_line = "$GPGSA,A,3,23,26,30,09,16,05,,,,,,,2.03,1.14,1.68*05\r\n" \
                         "GPGSV,2,1,06,05,15,302,32,09,80,069,37,16,21,063,39,23,43,077,37*7E\r\n" \
                         "GPGSV,2,2,06,26,13,034,20,30,25,194,25*73\r\n" \
                         "GPGLL,5227.42504,N,01317.81601,E,100137.00,A,A*62"
            self.ser.write(first_line)
            self.ser.write(second_line)
            self.ser.write(third_line)
            self.ser.write(final_line)
            print(first_line)
            print(second_line)
    ...
    def parser(self):   #, timestring
        ...
        self.time['year'] = time.year
        self.time['month'] = time.month
        self.time['day'] = time.day
        self.time['hour'] = time.hour
        self.time['minute'] = time.minute
        self.time['second'] = time.second
        self.time['usecond'] = time.microsecond
        print(self.time)
        return(time)
...
class RecorderStamper(threading.Thread):
   ...
   '''
    ...
    def run(self):
        ...
        self.csvsheet, self.csvwriter = self.open_csv_sheet(
            self.time2string(time_now_GPSbase, YMDHMSuS=[1, 1, 1, 1, 1, 1, 0]))
        ...
        if buf.startswith(b'\xff\xd8'):
            ...
            with self.picture_lock:
                if self.frame_index % (self.framerate * FOOTAGE_LENGTH) == 0:
                    self.folderName = RECORDING_PATH + "/" + self.time2string(self.gpsTimeNow(self.camera.timestamp),
                                                                              YMDHMSuS=[1, 1, 1, 1, 1, 1, 0])
                ...
                if not self.actual_GPS_time is None:
                    self.csvwriter.writerow(
                        [str(self.frame_index), str(self.actual_GPS_time["year"]), str(self.actual_GPS_time["month"]),
                         str(self.actual_GPS_time["day"]), str(self.actual_GPS_time["hour"]),
                         str(self.actual_GPS_time["minute"]), str(int(self.actual_GPS_time["second"])),
                         str(self.actual_GPS_time["usecond"])])
                else:
